  ...
  useEffect(() => {
    fetch("/api/dashboard")
      ...
      .then((data) => {
        setClientData(data.client_counts || {});
        setSlowData(data.slow_counts || {});
        setTimelineData(data.timeline || []); // 👈 backend returns [{client, timestamp, slow}]
    ...
    <div className="dashboard">
      ...
      <div className="top-metrics">
        <div className="metric-card">
          ...
          <p style={{ fontSize: "28px" }}>{totalClients}</p>
        </div>
      </div>
            ...
            data={[
              {
                ...
                values: Object.values(slowData),
                type: "pie",
                marker: { colors: ["#ef5350", "#26c6da"] },
              },
            ]}
            layout={{ title: "Slow vs Normal Queries", ...darkLayout }}
          />
        </div>
      </div>

      {/* 🔹 New Chart: Client vs Slow/Normal over Time */}
      <div className="time-chart">
        <Plot
          data={[
            {
              x: timelineData.map((d) => d.timestamp),
              y: timelineData.map((d) => (d.slow ? 1 : 0)),
              text: timelineData.map((d) => d.client),
              mode: "markers",
              type: "scatter",
              marker: {
                color: timelineData.map((d) => (d.slow ? "red" : "green")),
                size: 10,
              },
            },
          ]}
          layout={{
            title: "Slow vs Normal Queries Over Time",
            yaxis: { title: "Query Type", tickvals: [0, 1], ticktext: ["Normal", "Slow"] },
            ...darkLayout,
          }}
      ...
      {alerts.length > 0 && (
        <div className="alert-box">
          ...
          <ul>
            {alerts.map((a, i) => (
              <li key={i}>
                [{a.timestamp}] <strong>{a.client}</strong>: {a.message}
              </li>
            ))}
          </ul>
        </div>
